...
describe('Base32Encoder', () => {
  ...
  describe('when decoding', () => {
    it('can decode a multiple of 5 bits', () => {
      expect(Base32Encoder.decode('MVJP6D2Z').toString('hex')).toBe(
        'a6e563345f',
      );
    });

    it('can decode a single byte', () => {
      expect(Base32Encoder.decode('3M').toString()).toBe('t');
    });

    it('can decode a large number', () => {
      expect(Base32Encoder.decode('5JFW7B7M467TZ').toString('hex')).toBe(
        '593f8759e8431f5f',
      );
    });

    it('keeps leading zeros when decoding', () => {
      expect(Base32Encoder.decode('00059').toString('hex')).toBe('0000a9');
    });

    it('pads to the next byte', () => {
      expect(Base32Encoder.decode('M3kV').toString('hex')).toBe('0a0e7b');
    });
    ...
    `(
      ...
      ({ input, output }: { input: string; output: string }) => {
        expect(Base32Encoder.decode(input).toString('hex')).toBe(output);
      },
    );

    it('can decode a ULID', () => {
      expect(
        Base32Encoder.decode('01FJSVJEYB82V18ZBR2F2TT8SS').toString('hex'),
